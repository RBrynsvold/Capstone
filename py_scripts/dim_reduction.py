...
def create_save_objs(source_dir, outputs_dir, distinguishing_str, stop_words=1, min_freq=1):
    '''
...
def dim_red_counts(fileid_lst, all_transf_books_lst, dictionary, corpus):
    ...
    #changing this to avoid going thru full text again
...
def save_stuff(distinguishing_str, dictionary, corpus, outputs_dir):
    '''
    Save the outputs of the desired object(s)
    '''
    if dictionary != None:
...
if __name__=='__main__':
    #set source and output directories (with user prompting??)
    ...
    distinguishing_str='first_test'

    create_save_objs(source_dir, outputs_dir, distinguishing_str)
    #run most complete version of dim red by defaultdict
    #user can call intermediate dim red modules if needed
